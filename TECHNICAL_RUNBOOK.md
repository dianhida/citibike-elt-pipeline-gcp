...
This document is the **ground-truth setup guide** based on what’s in this repo (paths, env vars, mounts, and default names).

If you want a quick overview first, see:
...
| **dbt** | Run from host: use **`dbt/profiles.yml`** with **`keyfile:`** pointing to a **host path** (same JSON as above). Use `--profiles-dir dbt` (or `../` from `dbt/citibike_dbt`). | **`dbt/profiles.yml`** stays under **`dbt/`** on the host; Docker mounts the whole **`dbt/`** tree to **`/opt/airflow/...`**. Keep **`keyfile: /opt/airflow/keys/my-creds.json`** in the profile so paths match the container. The DAG uses `--profiles-dir /opt/airflow/dbt`. |
